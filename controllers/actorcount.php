...
if (!isset($get['accountid'])) return_json($response, 401);
...
if ($result->num_rows == 0) return_json($response, 401);
...
if ($get['hash'] != $hashed_secret && $get['hash'] != $hashed_secret_prev) return_json($response, 401);
...
function fetch_data($response) {
	global $get, $mysqli;
	if (!isset($get['city']) && !isset($get['state']) && !isset($get['country'])) return_json($response, 400);
	...
	if (isset($get["state"])) {
		...
		if ($result->num_rows == 0) return_json($response, 400);
